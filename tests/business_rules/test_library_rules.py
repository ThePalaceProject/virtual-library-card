...
class TestLibraryRules(BaseUnitTest):
    ...
    def test_validate_address_valid_state(self):
        ...
        result = LibraryRules.validate_user_address_fields(library, us_state="AL")
        assert result.us_state_valid == True
        assert result.zip_valid == True  # by default

    def test_validate_address_invalid_state(self):
        library = self.create_library(us_states=["NY", "AL"])
        result = LibraryRules.validate_user_address_fields(library, us_state="HI")
        assert result.us_state_valid == False
        assert result.zip_valid == True  # by default
    ...
    def test_validate_address_valid_zip(self):
        ...
        result = LibraryRules.validate_user_address_fields(
            library, us_state="AL", zip=99999, city="somecity"
        )
