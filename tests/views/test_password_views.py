...
class TestPasswordResetView(BaseUnitTest):
    ...
    def test_reset_email_with_no_cards(self):
        ...
        response = self.client.post("/account/reset-password/", {"email": user.email})
    ...
    def test_reset_email_with_two_cards(self):
        ...
        LibraryCard.objects.filter(user=user).delete()
        card1 = self.create_library_card(
            user, self._default_library, number="111111111"
        )
    ...
    def test_reset_email_excludes_canceled_cards(self):
        ...
        active_card = self.create_library_card(
            user, self._default_library, number="ACTIVE123"
        )
        canceled_card = self.create_library_card(
            user, self._default_library, number="CANCELED456"
        )
