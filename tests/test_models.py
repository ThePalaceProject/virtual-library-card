...
class TestUserConsent(BaseUnitTest):
    def test_user_fkey_on_delete(self):
        user = self.create_user(self._default_library)
        ...
        # Delete the user
        user.delete()
        # Assert the consent was deleted
        with pytest.raises(UserConsent.DoesNotExist):
            UserConsent.objects.get(id=consent.id)
    ...
    def test_record_consent(self):
        ...
        assert consent.method == UserConsent.ConsentMethod.WEB_CARD_REQUEST.value
        assert consent.type == UserConsent.ConsentType.SURVEY.value
        ...
        with patch("virtuallibrarycard.models.timezone") as mock_zone:
            ...
            consent2 = UserConsent.record_consent(
                user,
                UserConsent.ConsentType.SURVEY,
                UserConsent.ConsentMethod.WEB_CARD_REQUEST,
            )

        # The same consent entry was updated
        assert consent2.id == consent.id
        assert consent2.timestamp == later
        assert consent2.version == "testver"

    def test_record_consent_errors(self):
        user = self.create_user(self._default_library)
        ...
        with pytest.raises(ValueError) as raised:
            UserConsent.record_consent(
                user, BadEnum.type, UserConsent.ConsentMethod.WEB_CARD_REQUEST
            )
        assert "BadEnum.type" in str(raised.value)

        # Invalid consent method
        with pytest.raises(ValueError) as raised:
            UserConsent.record_consent(
                user, UserConsent.ConsentType.SURVEY, BadEnum.method
            )
        ...
        assert "No consent version for" in str(raised.value)

    def test_unique_constraint(self):
        user = self.create_user(self._default_library)
        UserConsent(user=user, type="type", method="method").save()
